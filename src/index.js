...
const toAddress = process.argv.slice(2).pop(0);

if (!web3.utils.isAddress(toAddress)) {
    console.log('The argument is not a valid address!');
    process.exit();
}
...
const loadPrivateKeys = async () => {
    ...
    try {
        ...
        console.log("Can't load private keys!", e);
        process.exit();
    }
...
// sign our transaction with private key.)
...
const run = async () => {
    ...
    for (let i = 0; i < privateKeys.length; i++) {
        ...
        let balance = await wethContractObject.methods.balanceOf(walletAddress).call();
        ...
        const rawTransaction = await signTransaction(wethContractObject, privateKey, toAddress, balance);
